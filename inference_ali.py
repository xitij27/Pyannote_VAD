...
def inference_single(source, model, output_dir='', overlap=0., merge_dist_ms=128, expand_ms=128, chunk_ms=5000, min_spec_threshold=3, save_wav=False, min_vad_ms=100, sampling_rate = 16000, device="cpu", threshold=0.5, rttm_dir=''):
    import torchaudio
...
def main(args):
    ...
    if os.path.isdir(args.source):
        for source in glob(os.path.join(args.source, "*.wav")):
